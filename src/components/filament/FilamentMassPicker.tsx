...
function MassPreset({ value, onClick }: { value: number, onClick: (v: number) => void }) {
    return <div className={`bg-bg-lighter rounded-full px-2 py-1 cursor-pointer border-2 border-transparent hover:border-primary
    transition-all w-full text-center select-none`} onClick={() => onClick(value)}>
        {grams(value)}
    </div>;
}
